...
<option value="KANTOR REGIONAL XIV MANOKWARI">KANTOR REGIONAL XIV MANOKWARI</option>
    ...
    <form method="GET" class="form-inline mb-3">
        <input type="text" name="search" value="<?= htmlspecialchars($search_query) ?>" class="form-control mr-2" placeholder="Cari Perihal">
    ...
    <table class="table table-bordered">
        <thead>
            <tr>
                <th>Nomor Memo</th>
                <th><a href="?search=<?= htmlspecialchars($search_query) ?>&sort=<?= $sort_order == 'asc' ? 'desc' : 'asc' ?>">Tanggal <?= $sort_order == 'asc' ? '↑' : '↓' ?></a></th>
                <th>Perihal</th>
                <th>Tujuan</th>
                <th>Dokumen</th>
                <th>Aksi</th>
            ...
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        ...
                        <a href="edit_surat_penugasan.php?id=<?= $row['id'] ?>" class="btn btn-warning btn-sm">Ubah</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </tbody>
        ...
        <ul class="pagination">
            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link" href="?page=<?= $i ?>&search=<?= htmlspecialchars($search_query) ?>&sort=<?= $sort_order ?>"><?= $i ?></a>
